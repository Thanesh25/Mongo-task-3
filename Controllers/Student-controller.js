...
import Mentors from "../Models/Mentor-Model.js";
...
export const createStudent = async (req, res) => {
  ...
  try {
    if (!name || !email) {
      return res
        .status(400)
        .json({ message: "All fields are required(name and email)" });
    }
...
export const assignStudentsToMentor = async (req, res) => {
  try {
    ...
    if (!mentorId || !studentsId) {
      return res
        .status(400)
        .json({ message: "All fields are required(mentorId and studentsId)" });
    }
    ...
    const studentsWithoutMentors = await Students.find({
      _id: { $in: studentsId },
      mentor: null,
    });
    const studentsWithMentors = await Students.find({
      _id: { $in: studentsId },
      mentor: { $ne: null },
    });
    ...
    if (studentsWithMentors.length > 0) {
      ...
      return res
        .status(400)
        .json({
          message: `These students already have mentor`,
          studentsId: studentsWithMentorsIds,
        });
    }
...
export const changeMentor = async (req, res) => {
  try {
    ...
    if (!mentorId || !studentId) {
      return res
        .status(400)
        .json({ message: "All fields are required(mentorId and studentId)" });
    }
    const mentor = await Mentors.findOne({ _id: mentorId });
    if (!mentor) {
      return res.status(404).json({ message: "Mentor not found" });
    ...
    if (previousMentor._id == mentorId) {
      return res
        .status(200)
        .json({ message: "This mentor is already assigned" });
    }
    if (previousMentor) {
...
export const showPreviousMentor = async (req, res) => {
  try {
    ...
    const student = await Students.findOne({ _id: studentId }).populate(
      "mentor"
    );
    if (!student) {
      return res.status(404).json({ message: "Student not found" });
    ...
    if (student.mentor == null) {
      return res
        .status(200)
        .json({ message: "This student has no previous mentor" });
    }
    res.status(200).json({
      mentor: {
